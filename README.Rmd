...
The researcher may provide a weight ($z$) to be utilized with amplifiers/de-amplifiers (default is $.8$; de-amplifier weight is constrained to $-1$ lower bound).  Last, these weighted context clusters ($c_{ijl}$) are summed ($c'_{ij}$) and divided by the square root of the word count ($w_ijn$) yielding an unbounded polarity score ($\delta_{ij}$) for each sentence.
